...
Regional maps now expose whole-case counts for limit violations, incomplete assessments and model ambiguities above the map. Each category opens a paginated list (50 entries). Violations rank by relative exceedance, with zero limits first, and retain terminal identity, result value, supplied limit, absolute exceedance and case/result field paths. Selection opens the element, fits its structural network when coordinates exist, and selects the affected bus terminal in the voltage layer. Unplaced elements remain selectable with an explicit location notice.

Checks currently cover aligned per-terminal phase-to-ground `v_min`/`v_max` limits and line current limits derived from matching end-current samples and supplied conductor ratings. Flat voltage arrays require an established phase-to-ground reference. Reported loading alone is not accepted as proof of a supplied-rating violation. Case identity mismatches suppress numeric violations; unverified pairing remains visibly labelled. Unsupported phase-neutral, phase-pair and neutral limit mappings, invalid limits, missing linecodes and invalid connection ports are exposed separately. This is a bounded set of engineering checks, not a complete validation of every model field or operating constraint.

Issue indexes are cached per case, result and scenario, independent of viewport and colour-layer filtering. Validation passes 16 build/model checks, six browser suites, 68 Julia assertions, and all 26 private regional case probes. Browser tests exercise ranking, pagination, terminal selection, network fitting, unplaced equipment and mobile overflow. Private case data remains outside the repository.

### Investigation workspace and assessment lifecycle

See `ENGINEERING_REVIEW_PLAN.md` for the implemented design and acceptance criteria. Regional evidence now opens in a bounded drawer alongside the map (below it on narrow displays), with question presets and local search/scenario controls. Assessment indexes remain cached per case, result and scenario. No-result cases run model checks without creating an operating finding for every missing result. Passed and violated terminal/end checks are counted separately from unassessable checks, unsupported groups and model findings.

Shared strict interpretation removes the legacy small-voltage per-unit heuristic and missing-loading-to-zero conversion. Exports include result/reference/coverage context and embedded figure styles. The seven browser suites and private case probes retain bounded navigation. Springfield's regional overview uses 339 SVG descendants in the current probe; all 26 supplied cases pass without browser errors.
