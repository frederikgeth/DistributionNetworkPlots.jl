  ...
  function createSingleWireRenderer(dependencies) {
    ...
    function drawSingle() {
      ...
      for (const item of dependencies.overviewAssets().filter((e) => !e.connections?.length && e.ports?.length === 1)) {
        ...
        content += `<path d="M${p[0] + (left ? -38 : 38)} ${p[1]}H${x}V${y}" fill="none" stroke="${dependencies.colourOf(item.ref.kind)}" stroke-width="2" marker-end="url(#sld-arrow)"/>`;
        content += dependencies.singleSymbol(item, x, y);
      }
      for (const item of dependencies.overviewAssets().filter((e) => e.ref.kind === "transformer" && e.ports?.length > 2)) {
        ...
        points.forEach((p) => { content += `<path d="M${p[0]} ${p[1]}L${x} ${y}" fill="none" stroke="${dependencies.colourOf(item.ref.kind)}" stroke-width="2" marker-end="url(#sld-arrow)"/>`; });
        content += dependencies.singleSymbol(item, x, y);
      }
