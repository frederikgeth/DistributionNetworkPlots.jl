...
- Single-wire placement is topology-aware: the bus graph is classified once as radial (a tree or forest) or meshed (a genuine loop, meaning two distinct paths between the same pair of buses). Parallel branches and self-loops are collapsed before that test: a second cable on the same route adds no second path, and the renderer already fans parallel branches into their own lanes. Radial feeders get a tidy hierarchical tree — Buchheim, Jünger and Leipert's linear-time improvement of Walker's algorithm — so parents sit centred over the sections they feed and sibling subtrees never overlap. Meshed networks get breadth-first ranks from the feeder root with one barycentric ordering sweep per rank to reduce crossings. Both keep tree depth as the topology rank, honour the reading direction and selected root, and separate disconnected islands into their own bands. The status line names the classification and the strategy it selected.
...
The prototype intentionally has no external basemap or runtime backend. Its layout is deterministic by default. Dense single-line cases use a content-sized, scrollable canvas and preserve a minimum gap between buses in the same topology layer, rather than compressing every layer into the viewport. The Single-wire controls offer a stress arrangement — PivotMDS seeding SMACOF stress majorisation, refined only on components small enough for its O(n²) iterations — as well as the optional vendored ELK Layered browser bundle (`elkjs@0.10.2`); both persist their resulting bus positions locally. The stress layout is an explicitly selected exploratory mode, not the default: it has no random seeds and no animated simulation, picks its pivots by a fixed farthest-first traversal, and orients its principal axis along the reading direction so the feeder root still reads from the expected side. Layout profiles are cached per case identity, direction, and root bus; selecting a root passes a fixed-root preference to ELK and roots the tidy tree or the layered ranks. Manual bus nudges invalidate routed sections and return to deterministic paths. If the ELK bundle is unavailable, the topology-aware and stress layouts remain usable offline.
